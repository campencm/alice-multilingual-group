...
content_lemmas = [(t.lemma_.lower(), t.pos_) for t in fin_doc
...
df_freq = pd.DataFrame(
    [(language, total_fin_tokens, lemma, pos, count) for (lemma, pos), count in freq.items()],
    columns=["language", "token count", "lemma", "pos", "count"]
).sort_values("count", ascending=False)
